...
class Connector():
	...
	def __sendCommandAwait(self, command):
		if not self.ser.isOpen():
			self.ser.open()
		self.ser.write(command)
		i = 0
		while self.ser.isOpen() and i < 100:
			i += 1
			time.sleep(.010)
	...
	def readSensorType(self):
		result = int(self.__sendCommand(b'\x81', 2).hex(), 16)
		if result == 97:
			return 'Licht'
		elif result == 98:
			return 'Temperatuur'
	...
	def readDistance(self):
		return int(self.__sendCommandAwait(b'\x91').hex(), 16)
